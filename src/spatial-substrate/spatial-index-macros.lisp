...
(defmacro with-relevant-objects ((obj-bind-to obj add-test &rest args) &body body)
  (let ((obj-var (gensym))
	(result-var (gensym)))
    `(let ((,obj-var ,obj)
	   (,result-var nil))
       (with-new-level (level)
	 (unwind-protect
	     (with-selected-buckets (cur-bucket ,obj-var :overlaps ,@args)
	       (dolist (,obj-bind-to (elements cur-bucket) ,result-var)
		 (let ((tested? 
			(get-value-for ,obj-bind-to level))) ; Yes, No, NIL
		   (when (and (not tested?) ,add-test)
                     (setf ,result-var
			   (progn
			     (set-value-for ,obj-bind-to level 'yes)
			     ,@body))))))
